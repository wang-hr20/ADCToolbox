...
% freqCal is the fine tuned frequency from calibration
    ...
    addOptional(p, 'reltol', 1E-6, @(x) isnumeric(x) && isscalar(x) && (x > 0));
    ...
    if(rank([bits,ones(N,1)]) < M+1)
        warning('Rank deficiency detected. Patch enabled.');
        bits_patch = [];
        ...
        for i1 = 1:M
            ...
            elseif(rank([ones(N,1),bits_patch,bits(:,i1)]) > rank([ones(N,1),bits_patch]))  % column i1 of bits is non corr to the rest columns
                ...
                flag = 0;
                for i2 = 1:M2
                    if(abs(abs(corr(bits(:,i1),bits_patch(:,i2)))-1)<1E-3)    % correlated column found
                ...
                if(flag == 0)
                    error('Patch failed: cannot find the correlated column for column %d!',i1);
                end
            end          
        end
        [~,M] = size(bits_patch);
    ...
    A = [bits_patch(1:N,2:M),ones(N,1),xc,xs];
    b = -bits_patch(1:N,1);
    x = linsolve(A,b);
    ...
    if(fsearch)
        ...
        for ii = 1:100
            ...
            KC = ones([N,1]) * x(M+1:M+order)' .* order_mat;         % coefficient of cos
            KS = ones([N,1]) * x(M+1+order:M+order*2)' .* order_mat; % coefficient of sin
            xcd = -2*pi * KC .* time_mat .* sin(theta_mat*2*pi);    % cosine series partial freq
            xsd =  2*pi * KS .* time_mat .* cos(theta_mat*2*pi);    % sine series partial freq
            
            A = [bits_patch(1:N,2:M),ones(N,1),xc,xs,sum(xcd+xsd,2)];
            b = -bits_patch(1:N,1);
            x = linsolve(A,b);
            
            delta_f = x(end)*rate;
            err = rms(x(end)*A(:,end));
            relerr = err / sqrt(x(M+1)^2+x(M+1+order)^2);

            fprintf('Freq fine iterating (%d): freq = %d, delta_f = %d, err = %d, rel_err = %d\n',ii,freq,delta_f, err, relerr);
    ...
    weight = [1,x(1:M-1)'];
    weight = weight.*(10.^-MAG);    % post-scaling
    weight = weight(max(L,1)).*K;   % calculate merged column weight

    w0 = weight(find(weight,1));    % normalized to the first non-zero column
    weight = weight/w0;    
    ...
    ideal = -(xc * x(M+1:M+order) + xs * x(M+1+order:M+2*order))' /w0;
